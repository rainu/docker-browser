...
DOCKER_IMAGE="rainu/tor-browser"
...
DOCKER_NAME="tor-browser-$CUR_USER_ID"
...
showHelp() {
echo 'Starts the Tor-Browser docker container.

docker-tor-browser.sh [OPTIONS...]
...
Options:
    ...
    -x, --xarg
		Argument(s) for the underlying Tor-Browser
'
	exit 0
}
...
readArguments() {
	while [[ $# > 0 ]]; do
		...
		case $key in
		    ...
		    ;;
		    -x|-xargs)
		    TOR_BROWSER_ARGS=$TOR_BROWSER_ARGS" $2"
		    shift
		    ;;
		    -D|--docker)
...
execute docker run --rm -i $DOCKER_RUN_PARAMS $DOCKER_ARGS $DOCKER_IMAGE $TOR_BROWSER_ARGS
